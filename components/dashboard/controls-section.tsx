...
// Fluxo: botão Bot → lista catálogo do backend → selecciona
//        botão Engrenagem → configura parâmetros
//        botão Play → executa startCatalogBot()
// ============================================================
...
import type { CatalogBot } from '@/lib/use-nexora-ws'
...
// ─── Modal de parâmetros (engrenagem) ─────────────────────────
function ParamsModal({
  ...
  onClose: () => void
  onStart: (configOverride: Partial<BotConfig>) => void
}) {
  const strategyFields: StrategyFieldDef[] = STRATEGY_PARAMS_FIELDS[catalogBot.strategy] ?? []
  const commonFields: StrategyFieldDef[]   = COMMON_CONFIG_FIELDS

  // Inicializa com os valores padrão do catálogo
  const [cfgVals, setCfgVals] = useState<Record<string, unknown>>(() => {
    const v: Record<string, unknown> = {}
    commonFields.forEach(f => {
      v[f.key] = (catalogBot.defaultConfig as any)[f.key] ?? f.defaultValue
    })
    return v
  })
  ...
  const [prmVals, setPrmVals] = useState<Record<string, unknown>>(() => {
    ...
    strategyFields.forEach(f => {
      v[f.key] = (catalogBot.defaultConfig.strategyParams as any)?.[f.key] ?? f.defaultValue
    })
    return v
  })

  const handleStart = () => {
    const override: Partial<BotConfig> = {
  ...
  // Agrupar campos comuns por grupo
  const groups: Record<string, StrategyFieldDef[]> = {}
  commonFields.forEach(f => {
      ...
      <div className="fixed inset-x-4 top-4 z-50 max-w-lg mx-auto">
        <div className="bg-[#151b2e] border border-[#2a3142] rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-[#2a3142] bg-[#1a2235]">
            <div>
              <p className="text-white font-semibold text-sm">Configurar — {catalogBot.name}</p>
              <p className="text-gray-500 text-xs mt-0.5">{STRATEGY_LABELS[catalogBot.strategy]} · ajusta os parâmetros antes de iniciar</p>
            </div>
          ...
          {/* Corpo */}
          <div className="px-5 py-4 max-h-[62vh] overflow-y-auto space-y-5">

            {/* Parâmetros da estratégia */}
            {strategyFields.length > 0 && (
              <div>
                <p className="text-gray-500 text-[10px] font-bold uppercase tracking-wider mb-3">Parâmetros da Estratégia</p>
          ...
          <div className="px-5 py-4 border-t border-[#2a3142] bg-[#1a2235] flex gap-3">
            <button onClick={onClose}
              className="px-5 py-2.5 bg-[#2a3142] hover:bg-[#3a4255] text-gray-300 rounded-xl text-sm font-medium transition-colors">
              Cancelar
            </button>
            <button onClick={handleStart}
              className="flex-1 bg-[#22c55e] hover:bg-[#16a34a] text-white rounded-xl py-2.5 text-sm font-semibold transition-colors">
              ▶ Iniciar Bot
            </button>
          </div>
        </div>
...
export function ControlsSection() {
  ...
  const {
    catalogBots, isLoadingCatalog, sessionBots,
    wsStatus, lastError,
    listCatalogBots, startCatalogBot,
    stopBot, pauseBot, resumeBot,
    openTrades,
  } = useBots()
  ...
  // Bot seleccionado do catálogo
  const [selectedCatalog, setSelectedCatalog] = useState<CatalogBot | null>(null)
  // Instância em execução correspondente (do sessionBots)
  const [activeSession,   setActiveSession]   = useState<BotSummary | null>(null)

  const [animProgress, setAnimProgress] = useState(0)

  // Sincronizar instância activa com sessionBots
  useEffect(() => {
    if (!selectedCatalog) { setActiveSession(null); return }
    // Encontrar a instância mais recente do catálogo seleccionado em execução
    const running = sessionBots.find(b =>
      (b.status === 'running' || b.status === 'paused') &&
      b.name.startsWith(selectedCatalog.name)
    )
    setActiveSession(running ?? null)
  }, [sessionBots, selectedCatalog])
  ...
  // Play / Pause / Stop
  const handleToggle = () => {
    if (isRunning && activeSession)     { stopBot(activeSession.id);   return }
    if (isPaused  && activeSession)     { resumeBot(activeSession.id); return }
    // Não tem instância activa → abrir modal de parâmetros para iniciar
    if (selectedCatalog) setShowParams(true)
  }

  const handleStart = (configOverride: Partial<BotConfig>) => {
    if (!selectedCatalog) return
    startCatalogBot(selectedCatalog.id, selectedCatalog.name, configOverride)
  }
    ...
    <>
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          {/* Bot — lista o catálogo do backend */}
          <div className="relative">
              ...
              {selectedCatalog ? (
                <>
                  <span className="w-2 h-2 rounded-full shrink-0"
                    style={{ background: activeSession ? statusDot(activeSession.status) : '#4b5563' }} />
                  <span className="text-white font-semibold text-sm truncate">{selectedCatalog.name}</span>
                  <span className="text-gray-500 text-[10px] ml-auto shrink-0 capitalize hidden sm:block">
                    {STRATEGY_LABELS[selectedCatalog.strategy]}
                  </span>
                </>
              ) : (
                <span className="text-gray-400 text-sm">
                  {isLoadingCatalog ? 'A carregar…' : catalogBots.length === 0 ? 'Sem bots' : 'Selecionar'}
                </span>
              )}
            </button>

            {/* Dropdown — catálogo do backend */}
            {showBotDrop && catalogBots.length > 0 && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-[#1e2535] rounded-xl shadow-xl border border-[#2a3142] py-1 z-50 max-h-56 overflow-y-auto">
                {catalogBots.map(b => (
                  <button key={b.id}
                    onClick={() => { setSelectedCatalog(b); setShowBotDrop(false) }}
                    className={`w-full px-4 py-2.5 text-left hover:bg-[#2a3142] transition-colors ${selectedCatalog?.id === b.id ? 'bg-[#2a3142]/50' : ''}`}>
                    <div className="flex items-center gap-2">
                      ...
                      {b.tags?.length > 0 && (
                        <span className="text-[9px] text-[#3b82f6] bg-[#3b82f6]/10 px-1.5 py-0.5 rounded-full shrink-0">
                          {b.tags[0]}
                        </span>
                      )}
                    </div>
        ...
        {/* Engrenagem + Play/Pause + Steps */}
        <div className="flex items-center gap-4">

          {/* Engrenagem — configurar parâmetros do bot seleccionado */}
          <button
            onClick={() => selectedCatalog && setShowParams(true)}
            disabled={!selectedCatalog}
            title={selectedCatalog ? `Configurar ${selectedCatalog.name}` : 'Selecciona um bot primeiro'}
            className={`w-12 h-12 rounded-xl flex items-center justify-center border-2 transition-all shrink-0 ${
              selectedCatalog
                ? 'border-[#2a3142] bg-[#1e2535] hover:bg-[#2a3142] hover:border-[#3b82f6] text-gray-400 hover:text-[#3b82f6]'
                : 'border-[#2a3142] bg-[#1e2535] opacity-40 cursor-not-allowed text-gray-600'
            }`}>
            <Ico.Gear />
          </button>
          ...
          <button
            ...
            disabled={!selectedCatalog || wsStatus !== 'connected'}
            className={`w-12 h-12 rounded-xl flex items-center justify-center border-2 transition-all shrink-0 ${
              !selectedCatalog || wsStatus !== 'connected'
        ...
        {activeSession && (isRunning || isPaused || activeSession.status === 'error') && (
          <div className="flex flex-wrap items-center gap-3 px-3 py-2 rounded-lg bg-[#1e2535] border border-[#2a3142] text-xs">
            <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: statusDot(activeSession.status) }} />
            <span className="text-gray-300 font-medium truncate max-w-[100px]">{activeSession.name}</span>
            <span className="text-gray-600">·</span>
            <span className="text-white font-mono">{activeSession.stats.totalTrades} trades</span>
            <span className="text-[#22c55e] font-mono">{activeSession.stats.wins}W</span>
            <span className="text-gray-600">/</span>
            <span className="text-[#ef4444] font-mono">{activeSession.stats.losses}L</span>
            <span className={`font-bold font-mono ${activeSession.stats.netPnL >= 0 ? 'text-[#22c55e]' : 'text-[#ef4444]'}`}>
              {activeSession.stats.netPnL >= 0 ? '+' : ''}{activeSession.stats.netPnL.toFixed(2)} USD
            </span>
            {openCount > 0 && (
              <span className="text-[#f59e0b] font-medium">{openCount} aberto{openCount !== 1 ? 's' : ''}</span>
            )}
            {isPaused && (
              <button onClick={() => resumeBot(activeSession.id)}
                className="ml-auto text-[#3b82f6] hover:text-white transition-colors font-semibold">
                Retomar →
              </button>
            )}
      ...
      {/* Modal de parâmetros */}
      {showParams && selectedCatalog && (
        <ParamsModal
          catalogBot={selectedCatalog}
