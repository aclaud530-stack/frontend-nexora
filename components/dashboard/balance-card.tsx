...
export function BalanceCard() {
  ...
  return (
    <div className="rounded-xl overflow-hidden border-2 border-[#2a3142] shadow-lg" style={{ background: 'linear-gradient(135deg, #0d1117 0%, #1a1f2e 100%)' }}>
      <div className="flex">
        {/* Saldo */}
        <div className="flex-1 p-3 sm:p-4 bg-[#0d1117]/90 border-r border-[#2a3142]">
          ...
          <p className="text-white text-lg sm:text-xl md:text-2xl font-bold tracking-tight">
            <span className="text-gray-400">$</span> {displayBalance.toFixed(2)} <span className="text-xs sm:text-sm font-normal text-gray-400">{currency}</span>
          </p>
        </div>
        ...
        <div className="flex-1 p-3 sm:p-4 bg-[#1a1f2e]">
          ...
          <p className={`text-lg sm:text-xl md:text-2xl font-bold tracking-tight transition-colors duration-300 ${isProfit ? 'text-[#22c55e]' : 'text-[#ef4444]'}`}>
            <span className="text-gray-500">{isProfit ? '+' : '-'}$</span> {Math.abs(displayProfit).toFixed(2)}
          </p>
        </div>
      </div>
      ...
      <div className="px-3 sm:px-4 py-2 text-right bg-[#0d1117]/50 border-t border-[#2a3142]">
        <span className="text-gray-400 text-xs sm:text-sm font-medium">
          Operações <span className="text-[#22c55e] font-bold">{wins}</span> <span className="text-gray-600">/</span> <span className="text-white">{losses}</span>
        </span>
      </div>
    </div>
  )
